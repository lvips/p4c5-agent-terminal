/*
 * SPDX-FileCopyrightText: 2024-2026 Kevincoooool
 * SPDX-License-Identifier: Apache-2.0
 *
 * KSDIY ESP32-P4/C5 Development Board
 * Author : Kevincoooool (https://github.com/kevincoooool)
 * Store  : 淘宝店铺「酷世DIY」
 * GitHub : https://github.com/kevincoooool/ESP32P4_KSDIY
 *
 * 未经授权禁止用于商业倒卖。本代码仅供酷世DIY开发板用户学习使用。
 */
/*
...
static void compose_ap_ssid(void)
{
    ...
    if (esp_read_mac(mac, ESP_MAC_WIFI_SOFTAP) != ESP_OK &&
        ...
        /* ESP-Hosted (e.g. ESP32-P4 + companion): Wi-Fi MAC types are not
         * available before esp_wifi_start(). Fall back to the host chip's
         * base MAC from eFuse, which is always readable. */
        if (esp_read_mac(mac, ESP_MAC_BASE) != ESP_OK) {
            ESP_LOGW(TAG, "All MAC reads failed, SSID suffix will be 000000");
        }
    }
